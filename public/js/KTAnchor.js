    ...
    $.extend({
        ...
        KTAnchor: {
            ...
            showSlidMessage: function (message) {
                ...
                alert_elt.css({bottom: 30, opacity: 0, display: "block"}).animate({
                    bottom: 6,
                    opacity: 1
                }, 'fast', function () {
                    // 绑定界面点击时关闭弹出
            ...
            success: function (container, responseText) {
                ...
                // 请求到的文本
                    ...
                    // 查询日历的节点，并删除
                    $(".xdsoft_datetimepicker").empty().remove();
                    // 填充
                    $(container).empty();
            ...
            popupLoader: function (url) {
                ...
                // 特效
                if (popup_elt.data("on-show-set") === undefined) {
            ...
            setRequestProcess: function (process_length) {
                ...
                if (new_process_length <= full_process_length) {
                    setTimeout(function () {
                        $.KTAnchor.setRequestProcess(new_process_length)
                    }, 1);
                }
        ...
        confirm: function (message, success) {
            ...
            // 特效
            if (confirm_elt.data("on-show-set") === undefined) {
            ...
            if (success_btn.data("click-confirm") === undefined) {
        ...
        KTTreeMenuHTML: {
            ...
            getMenuHtml: function (menus_data, menu_level) {
                ...
                var menu_html = "";
                // 初始化菜单等级
                menu_level = (typeof(menu_level) !== "number") ? 0 : menu_level;
                // 开始循环
    ...
    $.fn.extend({
                ...
                $anchor.on("click", function () {
                    var anchor_action = function () {
                        ...
                        if (typeof($anchor.attr("container")) !== "undefined" && $anchor.attr("container").length > 1) {
                            // 如果不是填充到右边区域，就不能做后退操作
                            if (container !== $anchor.attr("container")) {
                                container = $anchor.attr("container");
                                $anchor.attr("pushstate", "no");
                            }
                        }
            ...
            $(this).find(":input").each(function (key, input_elt) {
                ...
                if (typeof(validation) === "string" && validation.length >= 1) {
                    ...
                    // 请输入邮箱
                        ...
                        if (!/^([0-9A-Za-z\-_\.]+)@([0-9A-Za-z\-_\.]+\.[a-z]{2,})$/g.test(input_value)) {
        ...
        htmlEditorBind: function () {
            $(this).find("textarea.ckeditor").each(function (key, elt) {
                var ckeditor_id = "html-editor-" + Math.floor(Math.random() * 1000000);
                var ckeditor_elt = $(this);
                ckeditor_elt.attr("id", ckeditor_id);
        ...
        uploadInputBind: function () {
            $(this).find("input.upload-input").each(function (key, input_elt) {
                ...
                upload_frame.css("background-image", "url(" + upload_input.attr("src") + ")");
                // 绑定事件
        ...
        KTPaging: function () {
            ...
            this.find(container).each(function (key, paging_bar) {
                ...
                $paging_elt.html('<nav><ul class="pagination"><li><span>' + (1 * total) + '#</span></li>' + paging_html + '</ul></nav>');
            });
            // 返回 JQuery 对象
            return this;
        },
...
function TagsInput(tags_input) {
    ...
    // 允许录入多个 tags
    this.multiple = typeof(this.tags_input.attr('multiple')) !== "undefined";
